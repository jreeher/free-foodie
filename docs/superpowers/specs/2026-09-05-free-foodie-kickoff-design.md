...
`app/_layout.tsx` (except the persister key, §1), `app/(auth)/*` (except copy strings), `lib/supabase.ts`, `lib/hooks/useAuth.ts`, `lib/hooks/useTheme.ts`, `lib/stores/uiStore.ts`, `lib/theme/*`, `lib/utils/dates.ts`, `fractions.ts`, `ingredients.ts`, `webCompat.ts`, `components/ui/*`, `components/recipe/IngredientRow.tsx`, `InstructionStep.tsx`, `ServingsAdjuster.tsx`, `supabase/functions/extract-recipe-url/`.

⚠️ **Correction found during implementation: `lib/stores/authStore.ts` cannot stay byte-for-byte unchanged.** Its `updateProfile` action is typed `Partial<Pick<Profile, 'display_name' | 'preferences' | 'household_id'>>` and calls a `update_user_preferences` RPC for preference-only updates — both `preferences` and `household_id` are Simmer Down household fields with no equivalent on the new minimal `profiles` table (§4), and the RPC doesn't exist in the new schema either. This is a hard compile error, not a style choice, once `database.types.ts` is rewritten (Task 5 of the implementation plan). Fix: narrow `updateProfile` to `Partial<Pick<Profile, 'display_name'>>` and always do a direct `.update()` — no RPC branch. `refreshProfile`'s logic (including its `PGRST116` fallback insert) needs no change; it never touched the removed fields. ⚠️ This also means the SECURITY DEFINER RPC pattern CLAUDE.md documents for working around React Native's `auth.uid()` RLS race condition is *not* carried forward for profile writes — that race is a real, previously-observed bug class in this codebase, so a direct `.update()` could in principle hit it again. Reintroducing an RPC preemptively for a single `display_name` field felt like scope creep for a kickoff with no evidence yet that it's actually a problem here; if display-name saves start failing silently on device, that RPC pattern is the fix.
